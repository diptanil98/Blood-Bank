...
<body >
    ...
    <form method="post" action="">
        <input placeholder="Enter your Location" type="text" name="location"style="padding:10px;border-radius:10px">
</form><br>
...
$sql="SELECT * from hospital where Location='$location'";
$result=$conn->query($sql);
if($result->num_rows>0){
    while($row=$result->fetch_assoc()){
        echo "<tr><td>".$row["S.no"]. "</td><td>".$row["Hospital_name"]."</td><td>".$row["Contact_no"]."</td><td>".$row["Location"]."</td><td>"."<button style='background-color:blue'><a href='donor.html'>Apply</a></button>"."</td><td>";
